...
from app.database.utils import PageParams, PaginationMeta, apply_filters
from app.database.utils import apply_sorting, generate_uuid, paginate
...
def test_paginate_applies_limit_and_offset() -> None:
    statement = paginate(select(DatabaseUtilityWidget), PageParams(page=3, page_size=10))
