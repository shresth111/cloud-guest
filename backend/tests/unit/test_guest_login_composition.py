...
One thing, end to end: with whitelist-only mode **off** (its shipped
default), an unauthenticated ``POST /api/v1/guest/login/otp`` from a guest
with **zero** matching ``guest_access``/``device_access`` rules returns
200 and an ``ACTIVE`` session.
...
def _build_app(fx, access_repository: RecordingAccessRepository) -> FastAPI:
    """The real ``guest_router`` with only leaf dependencies swapped.
    ...
    app.dependency_overrides[get_otp_service] = lambda: fx.otp_service
    ...
    app.dependency_overrides[get_mac_authorization_service] = lambda: None
...
async def _login(app: FastAPI, fx, *, device_mac: str | None) -> Response:
