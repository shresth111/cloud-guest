...
state; there is no history table, mirroring ``app.domains.firewall
.models.FirewallRule``'s identical "no live device push in this pass"
shape -- real RouterOS provisioning happens through ``app.domains
.network_config``'s existing push pipeline
(``renderers.render_content_filter_rule``), not this domain.
