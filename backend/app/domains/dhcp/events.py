"""Lightweight, in-process domain events for the DHCP Pool Management
module.

Mirrors ``app.domains.vlan.events``'s own identical design exactly: plain,
frozen dataclasses constructed by ``DhcpService`` methods and logged
...
@dataclass(frozen=True, slots=True)
class DhcpPoolUpdated:
    id: uuid.UUID
    occurred_at: datetime = field(default_factory=_now)


@dataclass(frozen=True, slots=True)
class DhcpPoolDeleted:
    id: uuid.UUID
    router_id: uuid.UUID
    occurred_at: datetime = field(default_factory=_now)
