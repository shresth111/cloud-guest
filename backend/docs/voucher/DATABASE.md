...
* **New** (no existing column anywhere): every column on `voucher_batches`/
  ...
  generation/approval/redemption lifecycle.
...
RBAC's scope tables), neither `voucher_batches` nor `vouchers` is
referenced by any RBAC scope column, so no RBAC follow-up migration is
needed here.
